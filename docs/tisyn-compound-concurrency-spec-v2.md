...
## 17. Changelog from v1.0.0

### v1.3.2 — Host crash and restart recovery semantics (§10.6)

Added §10.6 "Host Crash and Restart Recovery" as a new subsection immediately after §10.5 (Replay-to-Live Transition). No existing sections were changed.

**What was added:**

- **§10.6.1 Recovery Source of Truth.** Establishes that the durable journal plus the original IR are the sole recovery state. Transport sessions, in-memory state, and browser tab continuity are not required. Defines "observational reattachment" (UI reconnect) as distinct from semantic continuation, which is always driven by journal replay.

- **§10.6.2 Durable Yield Rule.** States the normative boundary: an effect with a durable `Yield` entry MUST NOT be re-dispatched; an effect without one is in-flight and MUST be eligible for re-dispatch. Applies uniformly to standard external effects and compound-effect child tasks.

- **§10.6.3 Partial-Journal Task Recovery.** Names crash/restart as a trigger for the existing §10.1–10.5 replay mechanism. Categorises the three task states on recovery (fully closed, partial journal, no journal) and refers to the existing replay machinery for their handling. No new mechanism is introduced.
...
### v1.3.1 — Race outcome classification, teardown wording, CompletionChannel clarification

#### §5.3 reverse teardown — normative property separated from mechanism

Rewrote §5.3 to distinguish the normative semantic requirement (children MUST close in reverse spawn order) from one particular mechanism for achieving it (serial halt-and-drain). The previous wording implied the serial approach was the only conforming strategy. The new wording states the close-order invariant as the requirement and describes serial halt-and-drain as one satisfying mechanism. Implementations MAY use other mechanisms provided the close-order invariant holds.

#### §7.3.1 `race([])` wording standardised

Unified the `race([])` description to read consistently as a protocol/runtime bug that produces `CloseReason: bug`. The previous text mixed "protocol error" (prose) with `CloseReason: bug` (code) without connecting them explicitly. Now reads: "`race([])` is a protocol/runtime bug … the runtime MUST close the parent task with `CloseReason: bug`."
...
`cancelled` child closes are classified as consequences, never primary outcomes.

#### §13.7 `orchestrate_race` — impossible-state tail classified as bug

Replaced `throw ProtocolError("race: no outcome")` with a `close_as_bug(...)` call that references §7.3.5 and carries an explicit description. `race([])` handling replaced with `close_as_bug(...)` for consistency with the taxonomy. Added cross-reference comment to `CompletionChannel` note.

#### `CompletionChannel` note — explicitly illustrative, not a required API

Rewrote the `CompletionChannel` note to state that it is an illustrative pseudocode abstraction. Conforming implementations do not need a type or API named `CompletionChannel`. Any mechanism is acceptable provided it satisfies two stated properties: (1) arrival-order observation, (2) journal invariant preserved (child `Close` recorded before parent `yieldIndex` advances).

#### §13.9 `complete_task()` — clarifying comment on child iteration

Added an inline comment to the child-wait loop clarifying that it is a "all children closed" barrier, not the mechanism that determines teardown order. Teardown ordering is enforced earlier by the runtime's halt/drain rules (§5.3, §8.4). The forward iteration does not weaken the reverse close-order guarantee.

---

### v1.3.0 — Pseudocode correctness and terminology cleanup

#### `orchestrate_all` rewritten (§13.6)

The previous version used a source-order join loop followed by post-hoc inspection, with the correct fail-fast behavior described only in a note. The note has been removed. The pseudocode now directly expresses the normative semantics:

- Child completions are observed in arrival order via `CompletionChannel`, not source-order joins.
- On the first observed child error, all remaining running siblings are immediately halted.
- All children are drained to `closed` before the parent resumes.
- The propagated error is selected from the lowest-index failing child after all children close (source-order, not arrival-order).

#### `orchestrate_race` rewritten (§13.7)

The previous version iterated `child_tasks` in source order with `yield* child_tasks[i].join`, which blocks on child 0 before observing later children. That is not race semantics: a later child cannot win if an earlier child has not closed yet. The pseudocode now correctly:

- Observes child completions in arrival order via `CompletionChannel`.
- Declares the first child to close with `ok` as the winner, regardless of its index.
- Immediately halts all remaining running siblings on winner determination.
- Drains all children to `closed` before resuming the parent.
- Selects the lowest-index failing child error in the all-fail path.

A note has been added explaining the `CompletionChannel` abstraction and the property it must satisfy: `next()` must not block on a specific child — it must yield whichever child has actually closed next.

#### `halt()` contract clarified (§5.2)

Added an explicit comment on the `halt()` interface method stating that it initiates the `cancelling` transition but does not synchronously drain the task to `closed`. Terminal drainage is observed via `ChildTask.join` within the caller's structured scope.

#### Remaining imperative "cancel" → "halt" (§7.3.3, §11.1)

- §7.3.3 step 4b: "Cancel all remaining running/cancelling siblings" → "Halt all remaining running siblings".
- §11.1 table: "Cancel running siblings", "Cancel siblings" → "Halt running siblings", "Halt siblings" in all four sibling-effect cells.

---

### v1.2.0 — Host verb renamed: `cancel` → `halt`

**Motivation:** The runtime should feel like a direct extension of Effection's task model. `halt` matches Effection's own vocabulary for stopping a task or scope. `cancel` reads as a generic futures/promise-oriented verb. The rename makes the intended substrate alignment explicit at the API boundary.

**Conceptual mapping preserved:**

- The runtime **halts** a task (`halt(taskId)`).
- The task enters the `cancelling` state.
- It eventually closes with `CloseReason: cancelled`.

The Tisyn task-state vocabulary (`running`, `cancelling`, `closed`) and close reason (`cancelled`) are **unchanged**.

**Changed in this revision:**

- `RuntimeTaskHost.cancel(taskId)` → `RuntimeTaskHost.halt(taskId)`.
- `signal_effection_cancel(task)` → `signal_effection_halt(task)` in pseudocode.
- `function cancel(task_id)` → `function halt(task_id)` in §13.8; recursive calls updated.
- All `RT->>Cx: cancel()` annotations in sequence diagrams → `halt()`.
- State diagram transition labels updated: `halt() called`, `halt() called again`.
- Idempotency prose updated: "`halt()` is idempotent" throughout §4.3, §8.1, §8.8, I-15.
- §8.6 section prose: "cancel signal" → "`halt()` call".
- §8.8 section title: "Nested Cancellation (Idempotent)" → "Nested Halt (Idempotent)".
- Checklist step 4 updated: `halt(task_id)`, "halt while blocked", "idempotent halt in all states".

---

### v1.1.0 — Architectural revision (from v1.0.0)

#### Breaking changes

- **All open questions closed normatively.** OQ-1 (teardown order), OQ-2 (all error selection), OQ-3 (race([])), OQ-4 (race error selection), OQ-5 (nested halt) are no longer open.

#### Architectural corrections

- **Strict structured concurrency framing added (§2).** The specification now explicitly grounds the design in Effection's strict structured concurrency model. Agent effects are defined as coroutine suspension/resumption points, not detached futures. This is the primary motivating frame for the design.
- **Promise-shaped interface removed.** `TisynEffectionAdapter`, `awaitAll()`, `awaitRace()`, `Promise<Result<...>>` have been removed. Replaced with Effection-native `Operation`, `spawn`, and `yield*` semantics.
- **`ChildTask.join` is an `Operation`, not a `Promise`.** The parent `yield*`s child joins within its own Effection scope, preserving structured lifetime.
- **`all`/`race` semantics remain in the Tisyn runtime.** They are not inside any adapter or host interface. `RuntimeTaskHost` only provides `spawnChild` and `halt` as low-level mechanics.
- **`TisynEffectionAdapter` renamed to `RuntimeTaskHost`.** The interface is now clearly identified as a Tisyn runtime abstraction, not a generic Effection adapter.
- **"Effection MUST provide…" language removed.** Replaced with "the runtime relies on Effection for…" and "the runtime MUST guarantee … regardless of substrate behavior."

#### Resolved open questions

- **OQ-1 (Effection teardown order):** Reverse child teardown is now a Tisyn runtime guarantee. The runtime enforces it regardless of substrate behavior. Normative statement added to §8.4 and §5.3.
- **OQ-2 / OQ-4 (error selection in `all` / `race`):** Both now use the same deterministic rule: propagate the error from the **lowest-index failing child**. Timing-dependent language removed.
- **OQ-3 (`race([])`):** `race([])` is now normatively a protocol error (`CloseReason: bug`). Never-resolving semantics explicitly rejected.
- **OQ-5 (nested halt):** `halt()` is explicitly idempotent. Calling it on a `cancelling` or `closed` task has no effect, even during cleanup.

#### Pseudocode revisions

- Generator-style notation (`yield*`) used throughout. Children are joined via `yield* ct.join` inside the parent's structured scope.
- `dispatch_effect` rewritten to make the coroutine suspension point explicit.
- `orchestrate_all` and `orchestrate_race` rewritten without Promise orchestration.
- Halting pseudocode updated for idempotency and explicit reverse ordering.
- `complete_task` updated to handle `cancelling` state override of close reason.

#### New sections

- §2: Strict Structured Concurrency and Tisyn (new).
- §14: Sequence Diagrams — six Mermaid sequence diagrams covering all major paths.
- §15: Task State Diagram — Mermaid stateDiagram covering all states and transitions including idempotent halt.

#### Invariant updates

- I-15 (`halt()` idempotent) added.
- I-16 (reverse teardown is runtime guarantee) added.
- I-17 (lowest-index error selection) added.
- I-18 (agent effects as coroutine suspension) added.
- Former I-12 (Effection lifetime boundary phrasing) updated to reflect that Effection is substrate, not co-author of invariants.

...
CP11. **Session replay semantics fixed.** CC5 no longer
      claims initialization alone reconstructs session
      state. Revised to say the session is rebuilt from
      the replayed execution history relevant to that
      session (initialization + prior plan effects), not
      from a separately persisted snapshot.

CP12. **Branch replay semantics fixed.** FK13 updated with
      the same correction — branch state is rebuilt from
      the replayed execution history for that branch, not
      initialization effects alone.

CP13. **EV2 unknown-event obligation corrected.** The MUST
      obligation for ignoring unknown types now falls on
      the runtime (open type set, opaque delivery), not
      on user-authored handlers. Authored handlers MAY
      choose how to handle unknown types.

CP14. **SV-011 conformance test corrected.** Replaced
      "Unknown event types are silently ignored by handler"
      with "Runtime delivers unknown event types to handler
      without rejection" — a system-valid property.

**v0.1.4 — Correction Pass 4**

CP8. **EV6 reconciled with ED1–ED3.** EV6 no longer says
     "events MUST NOT be guaranteed to be delivered" — that
     contradicted the buffered-from-start contract in §8.
     Revised to clarify the non-guarantee boundaries:
     events are not preserved across crash/recovery, not
     delivered if the handle is never supervised, and may
     differ on live-frontier re-establishment.

CP9. **Branch capability restrictions clarified.** Added
     CC8a stating that `ClaudeCodeBranch` follows the same
     restricted-capability rules as `ClaudeCodeCC` (CC6–
     CC7) unless explicitly distinguished.

CP10. **RD1 plan dispatch wording tightened.** "Child
      coroutineId allocation" replaced with "Child
      YieldEvents under execution coroutineId" — the
      actual journal mechanism.

**v0.1.3 — Correction Pass 3**

CP7. **Example naming corrected for handle/result
     distinction.** `result2` → `planned2` (§1 overview),
     `branchPlan` → `branchPlanned` (§1 overview),
     `altPlan` → `altPlanned` (Appendix A). All examples
     now consistently use `planned` / `*Planned` for
     execution handles and `result` / `*Result` /
     `*Proposal` for final values.

**v0.1.2 — Correction Pass 2**

CP6. **Event envelope propagated to all examples.** All
     code examples now use `event.type` and `event.data`
     per the normative envelope (EV1). Provider-shaped
     field access (`event.content`, `event.toolName`)
     replaced with `event.data`-based access patterns.
     Appendix A worked example updated with explicit
     narrowing from the opaque `data` field.

**v0.1.1 — Correction Pass 1**

CP1. **Fork contradiction resolved.** `ClaudeCodeCC` retains
     `fork()` on the root session capability. The branch
     return type is narrowed to `ClaudeCodeBranch` which
     exposes `plan()` only — no recursive `fork()`. FD4 is
     now consistent: branch-of-branch is deferred and the
     type system does not accidentally expose it. FK-001
     updated accordingly.

CP2. **Replay contract tightened.** RD5 now explicitly
     distinguishes two cases: (A) final result not yet
     journaled — replay reaches the live frontier and the
     plan execution is re-established live, with prior
     events permanently lost; (B) final result already
     journaled — replay returns the stored result without
     re-executing or regenerating events.

CP3. **Event taxonomy reduced.** EV2 no longer mandates a
     fixed provider-shaped event type list. The normative
     contract is now a minimal event envelope (`type` +
     `data`) with ordered delivery and unknown-type
     ignorability. Specific event types (`text`,
     `tool_start`, etc.) are moved to a non-normative
     examples table.

CP4. **Lowering overcommitment softened.** CC10 no longer
     freezes the exact IR lowering form. It specifies the
     semantic requirement (resource-creating operation)
     without mandating a particular compiler strategy.

CP5. **Vague timing language removed.** "Promptly" removed
     from ED3 and surrounding text. Buffering contract is
     now stated normatively without qualification.

**v0.1.0 — Initial Draft**

Design decisions treated as given:

D1. `ClaudeCode().open(...)` returns a session-scoped Claude
    Code capability. Users do not separately call
    `useClaudeCodeAgent(session)`.

D2. `cc.plan(...)` returns an execution handle — a genuinely
    new restricted capability type, distinct from spawn task
    handles and stream subscription handles.

D3. The execution handle supports exactly two authored uses:
    direct join (`yield* planned`) and supervised consumption
    (`yield* supervise(planned, handler)`). Single-consumer
    ownership — not both, not twice.

D4. `supervise` takes a structured per-event generator
    operation, not an unstructured callback.

D5. Events are non-durable and observational. The final
    result is the only durable artifact.

D6. `cc.fork()` creates an isolated child session for
    exploratory branching. Merge/promote is deferred.

D7. `accept()` is deferred. This spec covers Plan Mode-
    backed planning only.

D8. Enforcement model: compiler-first static enforcement,
    runtime-backstop for dynamic misuse.

D9. Event delivery contract: buffered from execution start.

...
| **CT-USECONFIG** | `@tisyn/runtime` | `useConfig()` exposes resolved projection |
...
| CFG-RIV-005 | P0 | Integration | V16 | `run.module` pointing to nonexistent file → startup failure |
| CFG-RIV-006 | P0 | Integration | V17 | Transport module that does not resolve → startup failure |
...
| CFG-REF-004 | P0 | Integration | §3.3 | `module` omitted → runtime resolves export from descriptor module |
| CFG-REF-005 | P0 | Integration | §3.3 | `module` specified → runtime imports separate module |
...
| ID | P | Type | Spec | Assertion |
| --- | --- | --- | --- | --- |
| CFG-USE-001 | P0 | Behavioral | R1 | `useConfig()` returns post-overlay config |
| CFG-USE-002 | P0 | Behavioral | R1 | `useConfig()` returns post-resolution config (env nodes resolved) |
| CFG-USE-003 | P0 | Behavioral | R2 | Return value contains no `EnvDescriptor` nodes |
| CFG-USE-004 | P0 | Behavioral | R3 | Return value does not contain invocation-time workflow arguments |
| CFG-USE-005 | P0 | Behavioral | R1 | Workflow does not observe intermediate descriptor forms |
| CFG-USE-006 | P1 | Behavioral | §7.5.2 | Return value MAY omit `entrypoints` |
| CFG-USE-007 | P1 | Behavioral | §7.5.2 | Return value MAY omit `tisyn_config` discriminants |
| CFG-USE-008 | P1 | Behavioral | §7.5.2 | Return value MAY omit `WorkflowRef` metadata |
| CFG-USE-009 | P0 | Behavioral | R3 | Invocation args delivered via function parameter, not `useConfig()` |
...
| --- | --- | --- | --- |
| A. Constructors | 16 | 1 | 17 |
| B. Validation (V1–V10) | 19 | 0 | 19 |
| C. Compiler-integrated (V11–V13) | 0 | 3 | 3 |
| D. Runtime-integrated (V14–V17) | 6 | 0 | 6 |
| E. Workflow reference | 6 | 0 | 6 |
| F. Entrypoint overlays | 9 | 1 | 10 |
| G. Environment model | 16 | 0 | 16 |
| H. Secret handling | 5 | 0 | 5 |
| I. `useConfig()` | 6 | 3 | 9 |
| J. Resolution order | 5 | 0 | 5 |
| K. Worked-example fixtures | 7 | 0 | 7 |
| **Total** | **95** | **8** | **103** |

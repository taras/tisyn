...
| MR-002 | Core | §9.5 | Middleware executes identically at the replay frontier | Install logging middleware. Execute workflow to completion (journal fully durable). Replay from the complete journal. Separately: crash partway through and recover from the partial journal. | On pure replay from a complete journal, the probe fires zero times — every caller yield has a matching stored entry that short-circuits dispatch per §9.5. On live original execution, the probe fires once per caller yield. On recovery from a partial journal, the probe fires exactly once per un-journaled yield (the replay frontier forward) and zero times per stored yield consumed before the frontier |
...
| MR-004 | Extended | §9.5 | Runtime applies the per-cursor short-circuit uniformly | Install middleware that appends to a log. Execute workflow. Replay from the complete journal. | Log entries appear in original execution and in recovery from the replay frontier forward. For yields whose stored entries short-circuit dispatch, the runtime MUST NOT invoke middleware — this is the normative short-circuit defined in §9.5, not an implementation-specific skip. The runtime does not gate middleware on any other basis (no feature flag, no environment variable, no per-middleware opt-in) |

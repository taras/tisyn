...
| MR-002 | Core | §9.5 | Middleware executes identically on every run | Install logging middleware that appends to a probe. Execute workflow to completion (journal fully durable). Replay from the complete journal. | The probe fires once per caller yield on the original execution AND once per caller yield on pure replay. Middleware-entry counts are identical between original and replay. Agent side-effect counters (live external IO at `runAsTerminal` delegation sites) increment on the original run and by 0 on replay — the runtime terminal boundary substitutes stored results in place of `liveWork()` re-execution |
...
| MR-004 | Extended | §9.5 | Runtime does not gate middleware based on replay phase | Install middleware that appends to a log. Execute; replay. | Log entries appear identically on both passes — the runtime does not skip, batch, or selectively suppress middleware based on replay phase (no feature flag, no environment variable, no per-middleware opt-in). Only `runAsTerminal(...)`-wrapped live work is replay-substituted; middleware itself always executes |

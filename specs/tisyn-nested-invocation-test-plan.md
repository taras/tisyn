...
- **`tisyn-scoped-effects-specification.md` §9.5** and its Core-tier companion test **MR-002** — the replay semantics: middleware re-executes identically on every run; the runtime terminal boundary substitutes stored results for `runAsTerminal(...)`-wrapped live work in place of re-firing external side effects. Inherited by direct citation in nested-invocation spec §8.2.
...
- **Tier.** 1.
- **Purpose.** Operationalize MR-002 for nested invocation under scoped-effects §9.5: invoking middleware re-executes identically on every run (original, pure replay, crash recovery); child agents are not re-dispatched for journal-complete effects because the runtime terminal boundary substitutes stored results at `runAsTerminal(...)`-wrapped live work.
- **Setup.**
  - T01 fixture. Install a harness middleware-entry probe (H-A5) recording each entry into the invoking middleware body.
  - Agent side-effect counter from H-A4 — counts live terminal-work executions.
...
- **Assertions.**
  - Original run: middleware-entry probe records at least one entry per caller yield. Agent side-effect counter increments per live-dispatched effect.
  - Pure replay from a complete journal: middleware-entry probe records at least one entry per caller yield (identical to original). Agent side-effect counter increments by **0** — the runtime terminal boundary substitutes stored results at `runAsTerminal(...)` delegation sites.
  - Recovery from partial journal: middleware-entry probe records entries identically for all caller yields (both durable and live-frontier). Agent side-effect counter increments only for the live-frontier portion.
  - `yield* invoke(...)` returns the same value on the original run and on replay / recovery.
- **Validates.** Spec §8.2; §8.5; H6; scoped-effects §9.5.
...
- **Invoking middleware re-executes on replay under MR-002 / §9.5 inheritance.** Covered by T13. Spec §8.2; H6; scoped-effects §9.5. External agent side effects at `runAsTerminal(...)`-wrapped terminals are replay-substituted by the runtime terminal boundary.
...
- **T13 (`middleware_reexecutes_on_replay_with_invoke`) is Tier 1.** It is the direct operationalization of MR-002 (Core tier in the scoped-effects companion test plan) under scoped-effects §9.5, applied to middleware that contains `invoke`. The underlying property is already Core-tier normative; the inheritance is explicit in nested-invocation spec §8.2. Classifying T13 as Tier 2 would under-state the conformance requirement.

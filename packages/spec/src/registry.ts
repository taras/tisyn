...
function isSpec(m: NormalizedSpecModule | NormalizedTestPlanModule): m is NormalizedSpecModule {
...
export function buildRegistry(
  ...
  for (const s of allSpecs) {
    ...
    if (prior !== undefined) {
      throw new Error(`buildRegistry: duplicate id "${s.id}" — ${prior} and spec share this id`);
    }
  ...
  for (let i = 0; i < dependencyOrder.length; i++) {
    orderIndex.set(dependencyOrder[i]!, i);
  }
  ...
  for (const loc of allRules) {
    installWithPrecedence(ruleIndex, loc.rule.id, loc);
  }

  const termIndex = new Map<string, TermLocation>();
  for (const loc of allTerms) {
    installWithPrecedence(termIndex, loc.definition.term, loc);
  }

  const conceptIndex = new Map<string, ConceptLocation>();
  for (const loc of allConcepts) {
    installWithPrecedence(conceptIndex, loc.concept.name, loc);
  }

  const errorCodeIndex = new Map<string, ErrorCodeLocation>();
  for (const loc of allErrorCodes) {
    installWithPrecedence(errorCodeIndex, loc.errorCode.code, loc);
  }

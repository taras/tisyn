/**
 * Vendored from `@effectionx/context-api` (PR #215, commit `7b570a9` on
 * branch `feat/context-api-custom-groups` at
 * https://github.com/thefrontside/effectionx). See the package LICENSE
 * for MIT attribution.
 *
 * The code is intentionally a verbatim port — no feature additions or
 * behavioral changes. Tisyn consumes it as the substrate for middleware
 * composition in `@tisyn/effects`, `@tisyn/agent`, and `@tisyn/runtime`.
 */
...
export interface Api<A, Group extends string = "max" | "min"> {
  ...
  around: (around: Partial<Around<A>>, options?: { at: Group }) => Operation<void>;
}
...
type ScopeMiddleware<A, Group extends string> = Record<Group, Partial<Around<A>>[]>;
...
export function createApi<A extends {}, Group extends string = "max" | "min">(
  ...
  let rawGroups = (options?.groups ?? DEFAULT_GROUPS) as readonly MiddlewareGroup<Group>[];
  ...
  let groupByName = new Map<string, GroupDefinition<Group>>(groups.map((g) => [g.name, g]));
  ...
  function cloneState(current: ScopeMiddleware<A, Group>): ScopeMiddleware<A, Group> {
  ...
  let context = createContext<ScopeMiddleware<A, Group>>(`$api:${name}`, emptyState());
  ...
  function* around(middlewares: Partial<Around<A>>, options?: { at: Group }): Operation<void> {
    ...
    if (!group) {
      ...
      throw new Error(`context-api "${name}": unknown group "${at}". Known groups: ${known}`);
    }

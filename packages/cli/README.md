...
Workflow target resolution stays separate from descriptor-module loading and uses a three-step dispatch:

- if `run.module` points to a JavaScript-family file, the CLI loads the pre-compiled module and extracts the named export
- if `run.module` points to a TypeScript file with a compiler-generated banner (`// Auto-generated by @tisyn/compiler`), the CLI loads it at runtime as pre-compiled output
- if `run.module` points to an authored TypeScript source file, the CLI compiles it via `compileGraph` and extracts the IR directly
...
Compile a workflow module graph into a generated TypeScript module. Accepts one or more root source files.

```bash
tsn generate src/workflow.ts -o src/workflow.generated.ts
tsn generate src/contracts.ts src/workflow.ts -o src/workflow.generated.ts
```
...
3. Resolves the workflow target:
   ...
   - explicit TypeScript with compiler banner -> bootstrap module loading (pre-compiled)
   - explicit TypeScript source -> compile via `compileGraph`, extract IR directly

...
- `yield* Config.useConfig(Token)` is the recognized authored form for config access, compiling to `ExternalEval("__config", Q(null))`. It requires exactly one `ConfigToken` identifier argument (UC1 if wrong arity, UC2 if non-identifier). Bare `useConfig(Token)` without the `Config.` namespace is rejected (UC3). The token provides static typing via `ConfigToken<T>` and is erased by the compiler. At runtime, the `__config` effect returns the resolved config projection from the execution-scoped config context.

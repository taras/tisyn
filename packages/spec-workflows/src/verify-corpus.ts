...
//
// Entrypoint:
//   pnpm exec tsn run packages/spec-workflows/src/verify-corpus.ts \
...
// `--target` flag. The target name is resolved through the
// `@tisyn/spec` manifest (`acquireCorpusRegistry({ specIds: [target] })`
// in `corpus-agent.ts` and `acquireFixture(target, kind)` in
// `filesystem-agent.ts`); adding a new target later is one manifest
// entry, nothing in this file.
//
...
// replay trace to `packages/spec-workflows/src/.debug/verify-corpus.ndjson`
...
// if you want a fresh run. At runtime, the workflow's Step 0 reads
// the resolved journal from the runtime's ConfigContext via
// `yield* Config.useConfig(JournalToken)` and prints a
// `── journal ──` block on stdout announcing the active mode and
// (for the file backend) the resolved path, so operators see the
// live state instead of having to infer it from `-e debug`.
...
events will be reused, and each run starts from a clean slate.`,
  ...
  // Step 2 — acquire + render + compare spec and test plan through
  // the same gate. The corpus binding owns every call into `@tisyn/spec`
  // and throws on any acquisition / readiness / structural-gate failure,
  // so the workflow only sees compare verdicts here. `compareMarkdown`
  // is a coarse structural diff (H2 headings, test IDs, coverage refs,
  // relationship lines); Claude remains as a secondary semantic gate
  // for prose wording and table content.

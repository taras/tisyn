...
- Add `EnforcementContext` and `installEnforcement()` for non-bypassable cross-boundary enforcement wrappers (distinct from `DispatchContext` so child scopes cannot bypass parent restrictions)
